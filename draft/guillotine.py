...
from shared.league import POSITIONS, REGULAR_WEEKS, WEEKS
from shared.noise import SCORE_FLOOR_Z, SIGMA_CHAMP, SIGMA_WEEK, TEAM_SEASON_SIGMA, cdf, phi

from .pool import Player
from .settings import GUILLOTINE_SIMS
from .value import (
    Levels,
    combine_wire,
    my_wire,
...
def solve(
    ...
    for _ in range(GUILLOTINE_SIMS):
        ...
        for w in range(REGULAR_WEEKS):
            ...
            mass = max(cdf(z), 1e-12)
            o *= mass
            surv_by_week[w] += mass
            hazards[w] = phi(z) / (sigma * mass)
        surv = o
        finalist = alive[0]
        champ_bar = (
        ...
        win = max(cdf(z), 1e-12)
        o *= win
        hazards[REGULAR_WEEKS] = phi(z) / (SIGMA_CHAMP * win)
        o_total += o
        surv_total += surv
        win_num += surv * win
