...
"""Normalize the provider snapshots into one boards file, data/boards.json.
    ...
    uv run -m draft.sources.build_rankings
...
Every row is resolved onto the pool's Sleeper id (shared.names.Resolver): by supplied
...
from shared.names import Resolver, normalized_name
from shared.paths import BOARDS, POOL, SLEEPER_PROJECTIONS
from shared.paths import RAW_BOARDS as RAW
...
def main(argv: list[str] | None = None) -> int:
    ...
    resolver = Resolver(pool["players"])
    ...
    def resolved(rows: list[dict]) -> list[dict]:
        for row in rows:
            row["sleeper_id"] = resolver.resolve_row(row)
        return rows
    ...
    print(f"normalized {len(sources)} sources -> {BOARDS.name}", file=sys.stderr)
    return 0
