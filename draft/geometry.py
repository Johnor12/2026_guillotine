...
Each pick row carries: pick_no (1..pick_count, the array order), round, pick_in_round,
draft_slot (the board column), roster_id (the Sleeper roster that receives the player,
which is the acquirer for a traded pick), user_id / username, is_mine, status ('made'
or 'pending'), and for made picks the selection's sleeper_id (the join key back to
pool.json), name, position, team and is_keeper.
"""
