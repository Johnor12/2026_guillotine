...
from shared.league import TEAMS

from .pool import Player
from .settings import MY_SLOT, ROUNDS, TOTAL_PICKS, draft_order, pick_label, picks_for_slot
...
def load_board(
    ...
    Geometry disagreements with settings.py are reported rather than raised, so they land
    in `validation.problems` next to the numbers they broke; the fix is editing settings.py.
    """
    problems: list[str] = []
    ...
    for label, got, want in (
        ...
        if got is not None and got != want:
            problems.append(f"{source} says {label}={got}, settings.py says {want}")
    ...
    if my_slot != MY_SLOT:
        problems.append(f"{source} says my draft slot is {my_slot}, settings.py says {MY_SLOT}")
