...
starting shape (lineups expand in-season — shared.league.WEEKLY_SHAPES) using that week's
...
holds contributes one such body (settings.WEEK_WIRE_BODIES — one per position early, more
...
pool under my FAAB policy (FAAB_HOLD_WEEKS / FAAB_SPEND_WEEK): the undrafted
...
from .settings import FAAB_HOLD_WEEKS, FAAB_SPEND_WEEK, UNAVAILABLE_RATE, WEEK_WIRE_BODIES
...
def survivors(w: int) -> int:
    ...
    return TEAMS - 2 * min(w, REGULAR_WEEKS)
...
def my_wire(
    ...
    every tier once the saved budget is spent (FAAB_HOLD_WEEKS/FAAB_SPEND_WEEK)."""
