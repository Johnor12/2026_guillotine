...
non-zero exit. `draft/tests.py` covers the states a real draft.json cannot currently reach.
"""

from __future__ import annotations

from shared.league import DEDICATED_SLOTS, POSITIONS, STARTING_SLOTS, TEAMS, WEEKS

from .board import Board
from .pool import Player
from .settings import MAX_ITERS, ROUNDS, TOTAL_PICKS, draft_order, pick_label, picks_for_slot
...
def validate(
    ...
    check(
        len(full) == ROUNDS,
        f"{len(full)} picks for slot {board.my_slot}, want {ROUNDS}",
    )
    check(
    ...
    check(
        accounted == TOTAL_PICKS,
        f"board accounts for {accounted} picks, want {TOTAL_PICKS}",
    )
    check(
