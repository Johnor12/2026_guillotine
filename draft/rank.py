...
"""Roster-aware draft board for this league, with an optimal-drafter draft simulation.
    ...
    uv run -m draft.rank               # pool.json + draft.json + source boards -> rankings.json
    uv run -m draft.rank --report      # + board, convergence and recommendation on stderr

Scope is this league and nothing else; the league's shape lives in shared/league.py and
the draft's strategy knobs in draft/settings.py. The value input is `weekly_points` from `pool.json`: DraftSharks'
...
(`draft/pool.py`). The method, in one breath: this is a guillotine
...
through log P(winning the final) (`draft/guillotine.py`). What the waiver wire holds
...
(`draft/value.py`); the opponents and the bars see the survivors' equal split, my
roster sees the same pool under my FAAB policy (`draft/settings.py`, FAAB_HOLD_WEEKS).
`draft.json`, the live board, is the
simulation's starting state, not a filter (`draft/board.py`). Only my slot uses the
...
first-candidate plan out to the end of the draft (`draft/planning.py`).
...
from shared.paths import BOARDS, DRAFT, POOL, RANKINGS, SOURCE_MATCHES

from .board import load_board
from .convergence import converge
from .opponents import load_opponent_strategies
...
from .pool import blend_projections, load_pool
from .rankings import build_rankings
from .report import report_board, report_summary
from .settings import NOISE, ROLLOUT_SIMS, SIMS
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"cannot read {DRAFT.name}: {exc}; run draft.fetch", file=sys.stderr)
        return 1
    board, board_problems = load_board(draft_raw, players, DRAFT.name)
    try:
        opponents = load_opponent_strategies(players, board, draft_raw, SOURCE_MATCHES, BOARDS)
    except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
        print(
            f"cannot build opponent strategies: {exc}; run draft.sources.investigate",
            file=sys.stderr,
        )
        return 1
