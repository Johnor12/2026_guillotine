...
failure:
    ...
    1. draft.fetch                 Sleeper's draft API -> draft.json
    2. draft.sources.investigate   provider boards + draft.json -> data_source_matches.json
    3. draft.rank                  pool + draft + source matches -> rankings.json
...
Usage:
    uv run -m draft.refresh            # draft.json, data_source_matches.json, rankings.json
    uv run -m draft.refresh --report   # + every step's validation summary on stderr
"""
...
from . import fetch, rank
from .sources import investigate
...
def main(argv: list[str] | None = None) -> int:
    ...
    for number, (name, step) in enumerate(
        (("draft", fetch.main), ("investigate", investigate.main), ("rank", rank.main)), start=1
    ):
