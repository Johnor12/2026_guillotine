...
(shared/data/weekly_projections.json, joined to Sleeper ids through pool.json, or by name
...
exact for this slot chain (shared.league.SLOT_CHAIN), so the current week's
recommendation is the true optimum under the projections. The race simulator scores
every team-week the same way, with surprise inactives left to the score noise rather
than the availability cascade the draft valuation used; the bidding model's own lineup
arithmetic is compiled in season/waivers.py.
"""
...
from shared.names import normalized_name
...
def draftsharks_by_sleeper(pool: dict, weekly_raw: dict, directory: dict) -> dict[str, dict[str, float]]:
    ...
    for sid, meta in directory.items():
        by_name.setdefault((normalized_name(meta["name"], drop_suffix=True), meta.get("position")), []).append(sid)
    ...
    for row in unpooled:
        found = by_name.get((normalized_name(row["name"], drop_suffix=True), row["position"]), [])
...
def load_season(pool_path: Path, weekly_path: Path, league_path: Path) -> SeasonState:
    ...
    if live != want:
        problems.append(
            f"Sleeper's week-{week} roster shape {live} disagrees with shared/league.py {want}; "
            "edit WEEKLY_SHAPES / WEEK_ROSTER_SIZE"
        )
    if league["faab_budget"] != FAAB_BUDGET:
        problems.append(f"Sleeper says the FAAB budget is {league['faab_budget']}, shared/league.py {FAAB_BUDGET}")
    if league["reserve_slots"] != RESERVE_SLOTS:
        problems.append(f"Sleeper says there are {league['reserve_slots']} reserve slots, shared/league.py {RESERVE_SLOTS}")
    if league["waiver_clear_days"] != WAIVER_CLEAR_DAYS:
        problems.append(f"Sleeper says waivers clear after {league['waiver_clear_days']} day(s), shared/league.py "
