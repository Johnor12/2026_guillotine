...
"""Build pool.json, the ranker's player pool, from the three snapshots in data/.
    ...
    uv run -m shared.build_pool            # -> pool.json
    uv run -m shared.build_pool --report   # + join diagnostics on stderr
...
There is no shared id between DraftSharks and Sleeper, so the join is by name
(shared.names.Resolver). Team codes differ between the two (JAC/JAX, LVR/LV) and are
translated; DraftSharks' UNS/RK sentinels mean unsigned. --report prints every
last-name-plus-team join for eyeballing, since that is the tier where a wrong join
would be plausible enough to slip through.
...
from .paths import POOL, PROJECTIONS_HTML, SLEEPER_PROJECTIONS, WEEKLY_PROJECTIONS
...
def uniform_weeks(points: float, bye_week: int | None) -> list[float]:
    per_week = round(points / WEEKS, 2)
    return [0.0 if w == bye_week else per_week for w in range(1, WEEKS + 1)]
...
def build(report: bool) -> dict:
    ...
    resolver = Resolver([r for r in sleeper["players"] if r["position"] in POSITIONS])
    ...
    fallbacks: list[dict] = []
    for row in page:
        found, tier = resolver.resolve(row["name"], row["position"], sleeper_team(row["team"]))
        tiers[tier] += 1
        if found is None:
