...
"""Refresh the two projection snapshots build_pool.py reads. Manual step; hits the network.
    ...
    uv run -m shared.fetch_projections
...
from .paths import SLEEPER_PROJECTIONS, WEEKLY_PROJECTIONS
from .sleeper import API, LEAGUE_ID, get_json, score as league_score
...
LEAGUE_URL = f"{API}/league/{LEAGUE_ID}"
STATE_URL = f"{API}/state/nfl"
SEASON_URL = (
...
def fetch_sleeper(scoring: dict, season: str) -> None:
    rows = get_json(SEASON_URL.format(season=season))
    players = []
    for row in rows:
        stats = row.get("stats") or {}
        points = league_score(stats, scoring)
        if points <= 0:
            continue
        who = row["player"]
        players.append(
